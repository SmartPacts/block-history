...
// (hash, time, source) or nothing. A single mismatch is a defect and exits 2. Missing heights are
...
type Row = { height: number; source: string; hash?: string; time?: string; by?: string };
type ChainReport = { chain: string; from: number; to: number; heights: number; attested: number; backfilled: number; missing: number; mismatches: { height: number; field: string; node: string; module: string }[]; firstMissing: number[]; latest: number };
...
async function moduleRows(c: ChainId, a: number, b: number): Promise<Row[]> {
  ...
  for (let lo = a; lo <= b; lo += 500) {
    ...
    const rows: Row[] = await local(
      ...
      // the seconds-only {"time"} form, which would make an exact row look truncated.
      `(map (lambda (h:integer) (if (${MODULE}.has-block h) (let ((b (${MODULE}.get-block h))) (+ { "time": (format-time "%Y-%m-%dT%H:%M:%S.%vZ" (at 'time b)) } b)) { "height": h, "source": "missing" })) (enumerate ${lo} ${hi}))`,
      { chainId: c });
    out.push(...rows);
  }
  return out;
...
async function checkChain(c: ChainId, topByChain: Record<string, { height: number }>): Promise<ChainReport> {
  ...
  const rep: ChainReport = { chain: c, from, to, heights: to - from + 1, attested: 0, backfilled: 0, missing: 0, mismatches: [], firstMissing: [], latest: -1 };
  ...
  for (let i = 0; i < rep.heights; i++) {
    ...
    if (m.source === 'missing') { rep.missing++; if (rep.firstMissing.length < 10) rep.firstMissing.push(m.height); continue; }
    if (m.source === 'attested') rep.attested++; else if (m.source === 'backfilled') rep.backfilled++; else throw new Error(`chain ${c}: unknown source ${m.source}`);
...
async function main() {
  ...
  const tot = reports.reduce((t, r) => ({ heights: t.heights + r.heights, attested: t.attested + r.attested, backfilled: t.backfilled + r.backfilled, missing: t.missing + r.missing, mismatches: t.mismatches + r.mismatches.length }), { heights: 0, attested: 0, backfilled: 0, missing: 0, mismatches: 0 });
  if (JSON_OUT) { console.log(JSON.stringify({ api: API, module: MODULE, at: new Date().toISOString(), total: tot, chains: reports }, null, 2)); }
    ...
    console.log('chain   range               heights  attested  backfilled  missing  coverage   latest  mismatches');
    for (const r of reports) {
      const cov = r.heights ? (((r.attested + r.backfilled) / r.heights) * 100).toFixed(2) + '%' : '-';
      console.log(`${r.chain.padStart(5)}   ${String(r.from).padStart(8)}..${String(r.to).padEnd(8)}  ${String(r.heights).padStart(7)}  ${String(r.attested).padStart(8)}  ${String(r.backfilled).padStart(10)}  ${String(r.missing).padStart(7)}  ${cov.padStart(8)}  ${String(r.latest).padStart(7)}  ${r.mismatches.length}${r.firstMissing.length ? `   missing: ${r.firstMissing.join(' ')}${r.missing > 10 ? ' …' : ''}` : ''}`);
      for (const m of r.mismatches.slice(0, 5)) console.log(`        MISMATCH h${m.height} ${m.field}: node=${m.node} module=${m.module}`);
    }
    const cov = tot.heights ? (((tot.attested + tot.backfilled) / tot.heights) * 100).toFixed(3) : '-';
    console.log(`\ntotal   heights ${tot.heights}  attested ${tot.attested}  backfilled ${tot.backfilled}  missing ${tot.missing}  coverage ${cov}%  mismatches ${tot.mismatches}`);
